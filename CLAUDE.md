...
- `PhotoItem.fileHash` 在 `loadFolder()` 时由 `FileHasher` 填充（前 4 MB SHA-256），评价以此为全局唯一键，文件夹移动/重命名后评价依然有效
